...
--- MoonSharp does dumb stuff with strings, so we just don't bother with utf8.
...
---
--- Unpacks a table, accounting for the `n` field if it exists.
--- If not, it returns every numerically indexed value of a table. \
...
--- Packs a table for use with [Utils.unpack](lua://Utils.unpack)
---@generic T
...
function Utils.split(str, sep, remove_empty)
    ...
    while i <= #(str) do
        ...
        if Utils.sub(str, i, i + (#(sep) - 1)) == sep then
...
function Utils.hook(target, name, hook, exact_func)
    ...
    -- __index-based class system doesn't need this (probably)
    --[[
...
function Utils.randomAxis()
    ...
    table.insert(t, math.random(2), 0)
    return t
end
...
function Utils.randomPointOnBorder(x, y, w, h)
    if math.random() < 0.5 then
        local sx = (math.random() < 0.5) and x or x+w
        local sy = math.random(y, y+h)
        return sx, sy
    else
        local sx = math.random(x, x+w)
        local sy = (math.random() < 0.5) and y or y+h
        return sx, sy
    end
end
...
function Utils.pick(tbl, sort, remove)
    if sort then
        ...
        local i = indexes[math.random(#indexes)]
        if remove then
...
function Utils.pickMultiple(tbl, amount, sort, remove)
    ...
    for _=1,amount do
        local i = table.remove(indexes, math.random(#indexes))
        if remove then
